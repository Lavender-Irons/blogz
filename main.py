...
db = SQLAlchemy(app)
app.secret_key = 'l1a2v3e4n5d6e7r8'
...
class Blog(db.Model):
    ...
    def __init__(self, title, body, owner, pub_date=None):
        self.title = title
        self.body = body
        if pub_date is None:  #set date/time
            pub_date = datetime.utcnow() #utc world regulates clocks and times
        self.pub_date = pub_date 
        self.owner = owner #set date/time to who logged in
...
def signup():
    if request.method == 'POST':
        ...
        if password != verify:
            ...
            flash('Username and password must be more than 3 characters, please try again.', 'error')
        elif existing_user:
            flash('User already exists, try again.', 'error')
        else:
